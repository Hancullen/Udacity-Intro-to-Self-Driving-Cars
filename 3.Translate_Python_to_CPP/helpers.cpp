...
// we have to use .push_back for each row, then append each row to the vector
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
    ...
    float center_p = 1.0 - blurring;
    float corner_p = blurring / 12.0;
    float adjacent_p = blurring / 6.0;
    vector<vector<float> > window { {corner_p, adjacent_p, corner_p},
                                    {adjacent_p, center_p, adjacent_p},
                                    {corner_p, adjacent_p, corner_p} };
    int i, j, dx, dy;
    for(i=0; i < height; ++i){
        for(j=0; j < width; ++j){
            ...
            for(dx=-1; dx < 2; ++dx){
                for(dy=-1; dy < 2; ++dy){
                    ...
                    long new_i = ((i + dy) % height + height) % height;
                    long new_j = ((j + dx) % width + width) % width;
                    newGrid[new_i][new_j] += (mult * grid_element);
                }
            }
        }
    }
